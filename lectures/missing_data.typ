...
Les trois mécanismes MCAR, MAR et MNAR, définis ci-dessous, distinguent la
dépendance du manque aux valeurs observées ou non observées. Ce sont des
hypothèses de travail à discuter avec le contexte de collecte, et non des
catégories directement identifiables dans le tableau.

=== MCAR

#definition(title: [MCAR : manque complètement aléatoire])[
  La probabilité de manquer ne dépend ni des valeurs observées ni des valeurs
  manquantes.
]
...
#example(title: [Panne indépendante du phénomène mesuré])[
...
#definition(title: [MAR : manque aléatoire conditionnellement aux valeurs observées])[
  Conditionnellement aux valeurs observées, la probabilité de manquer ne dépend
  plus des valeurs manquantes. Elle peut donc varier avec les variables observées.
]

#example(title: [Non-réponse expliquée par les variables observées])[
  La probabilité qu'un revenu soit manquant peut dépendre de l'âge, du type
  d'emploi ou de la région, si ces variables sont connues. L'hypothèse MAR
  suppose qu'à valeurs identiques de ces variables, cette probabilité ne
  dépend plus du revenu manquant.
]
...
#definition(title: [MNAR : manque non aléatoire])[
  Même conditionnellement aux valeurs observées, la probabilité de manquer
  dépend encore de valeurs non observées, notamment de la valeur absente
  elle-même.
]

#example(title: [Non-réponse liée au revenu manquant])[
  Les personnes ayant un revenu très élevé peuvent refuser de le déclarer
  précisément parce que ce revenu est élevé, même à âge, emploi et région
  identiques.
]
...
#definition(title: [Imputation simple])[
  L'*imputation* remplace les valeurs manquantes par des valeurs plausibles.
  Une imputation est dite *simple* lorsqu'elle produit un seul jeu de données
  complété.
]
...
#definition(title: [Imputation multiple])[

...
#definition(title: [Finalité et proportionnalité])[
  La *finalité* est l'objectif auquel doit répondre l'analyse. La
  *proportionnalité* consiste à adapter le niveau de détail collecté et les
  moyens mobilisés à cet objectif, en tenant compte des risques pour les
  personnes concernées.
]

Collecter ou utiliser des données parce qu'elles sont disponibles ne suffit
pas. Il faut les relier à une question légitime.
...
#remark(title: [Retirer les noms ne suffit pas])[
...
Il faut donc expliciter le choix retenu et le relier au contexte.

#example(title: [Des coûts d'erreur différents selon l'usage])[
  Dans un modèle
